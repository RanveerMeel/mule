...
public class ApplicationModel {
  ...
  private void expandModules(Optional<ExtensionManager> extensionManager) {
    if (extensionManager.isPresent()) {
      createOperationRefEffectiveModel(extensionManager.get());
      createConfigRefEffectiveModel(extensionManager.get());
    }
  }

  private void createOperationRefEffectiveModel(ExtensionManager extensionManager) {
    // TODO(fernandezlautaro): MULE-10866 we should gather a subset of the extensions to execute the templating, specifically looking for the XmlExtensionModelProperty =]
    ...
    executeOnEveryMuleComponentTree(componentModel -> {
      for (int i = 0; i < componentModel.getInnerComponents().size(); i++) {
        ...
        Optional<ExtensionModel> extensionModel = extensionManager.getExtension(identifier.getNamespace());
        String identifierName = identifier.getName();
        if (extensionModel.isPresent()
            && extensionModel.get().getModelProperty(XmlExtensionModelProperty.class).isPresent()
            && !identifierName.equals(MODULE_CONFIG_GLOBAL_ELEMENT_NAME)) {
          // TODO(fernandezlautaro): MULE-10866 clean this before PR
          ...
          //
          OperationModel operationModel = extensionModel.get().getOperationModel(identifierName)
              .orElseThrow(() -> new IllegalArgumentException(format("The operation '%s' is missing in the module '%s'",
                                                                     identifierName, extensionModel.get().getName())));
          ComponentModel replacementModel = createOperationInstance(operationRefModel, extensionModel.get(), operationModel);
  ...
  private void createConfigRefEffectiveModel(ExtensionManager extensionManager) {
          ...
          for (int i = 0; i < componentModel.getInnerComponents().size(); i++) {
            ...
            if (extensionModel.isPresent()) {
              List<ComponentModel> replacementGlobalElements =
                  createGlobalElementsInstance(configRefModel, extensionModel.get());
